...
-- This fixes RLS policy errors and sets up all required tables
-- ============================================================
...
-- Note: These policies work with both client-side (auth.uid()) and server-side (service role) access
...
create policy "Users can create their own folders" 
    ...
    with check (
        owner_id = auth.uid() 
        or auth.uid() is null  -- Allow server-side inserts (service role bypasses RLS anyway)
    );
...
-- FROM pg_policies 
-- WHERE tablename IN ('folders', 'starred_items')
...
-- 3. Test folder creation in your app
-- ============================================================
